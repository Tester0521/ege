...
gg = [len(f'{[[s[x]+s[y] for y in range(x+1, len(s)) if (s[x] - s[y]) % 2 == 0 and (s[x] % 19 == 0 or s[y] % 19 == 0)] for x in range(len(s)-1)]}'.replace('[', '').replace(']', '').replace(',', '').split()), max([int(el) for el in f'{[[s[x]+s[y] for y in range(x+1, len(s)) if (s[x] - s[y]) % 2 == 0 and (s[x] % 19 == 0 or s[y] % 19 == 0)] for x in range(len(s)-1)]}'.replace('[', '').replace(']', '').replace(',', '').split()])]
...
s = [int(el) for el in open('./src/17.txt')]
'''count = m = 0
